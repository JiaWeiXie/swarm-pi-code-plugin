...
test("sandbox policy separates readonly and implementation write access", async () => {
  ...
  assert.equal(implement.filesystem.denyWrite.includes(path.join(canonicalWorkspace, ".git")), true);
...
test("sandbox policy canonicalizes symlinked workspace aliases", async () => {
  ...
  const tempRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "swarm-pi-sandbox-alias-home-")));
  fs.mkdirSync(path.join(cwd, "src"));
  ...
  const configuration = await sandboxConfiguration(alias, tempRoot, "implement", process.env, "lenient", [], bound);
