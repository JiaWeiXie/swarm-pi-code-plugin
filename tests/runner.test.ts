...
test("argument parsing requires host and prompt file for ask", () => {
  ...
  assert.throws(
    () => parseArguments(["configure", "--section", "models"]),
  ...
  assert.throws(
    ...
    /only supported by plan/,
  );
  assert.throws(
    () => parseArguments(["init", "--section", "project"]),
    /only supported by configure/,
  );
  assert.equal(parseArguments(["status", "--json"]).command, "status");
  assert.equal(parseArguments(["doctor", "--smoke-test", "--json"]).smokeTest, true);
  assert.equal(
    parseArguments(["resume", "--continuation", "00000000-0000-0000-0000-000000000000"])
      .continuationId,
        ...
        if (options.mode === "readonly") {
          listener({
            ...
            assistantMessageEvent: {
              type: "text_delta",
              delta: "VERIFIED: changes match the task",
            },
          });
        }
        listener({ type: "message_end", message: { role: "assistant", stopReason: "stop" } });
...
test("independent verification reuses the ordered Job candidates and contributes fallback attempts", async () => {
          ...
          if (mode === "readonly" && model.id === fallbackModel.id) {
            listener({
              ...
              message: {
                role: "assistant",
                stopReason: "error",
                errorMessage: "verifier candidate unavailable",
              },
            });
          } else {
            if (mode === "readonly") {
              listener({
                ...
                assistantMessageEvent: {
                  type: "text_delta",
                  delta: "VERIFIED: ordered fallback passed",
                },
              });
            }
            listener({ type: "message_end", message: { role: "assistant", stopReason: "stop" } });
  ...
  assert.equal(
    "agentVerification" in result && result.agentVerification?.model,
    "test-provider/test-model",
  );
...
test("implement returns actionable isolation strategies for a dirty worktree", async () => {
  ...
  assert.deepEqual("strategies" in result ? result.strategies : [], [
    "isolated-head",
    "isolated-snapshot",
  ]);
});
...
test("unborn implementation fails before model use and resumes after workspace repair", async () => {
      ...
      return {
        ...
        async prompt() {
          if (options.mode === "implement") {
            ...
            fs.writeFileSync(
              path.join(options.cwd, "src", "example.ts"),
  ...
  execFileSync("git", ["-c", "commit.gpgsign=false", "-C", workspace, "commit", "-m", "initial"], {
    stdio: "ignore",
  });
...
test("safe-dirty implementation is isolated and materializes without changing preserved files", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async (options) => ({
      ...
      async prompt() {
        if (options.mode === "implement")
          fs.writeFileSync(path.join(options.cwd, "tracked.txt"), "after\n");
      },
      dispose() {},
    }),
  };
  ...
  assert.equal(
    "artifact" in result && result.artifact?.worktree
      ? fs.existsSync(result.artifact.worktree)
      : true,
    false,
  );
});
...
test("mechanical executor escalates only after a side-effect-free failure", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async ({ model, mode }) => {
      ...
      return {
          ...
          if (mode === "readonly") {
            ...
            listener({
              ...
              message: {
                role: "assistant",
                stopReason: "error",
                errorMessage: "mechanical failed",
              },
            });
          } else {
            listener({ type: "message_end", message: { role: "assistant", stopReason: "stop" } });
        ...
        async prompt() {
          if (mode === "implement" && model.id === fallbackModel.id)
            fs.writeFileSync(path.join(workspace, "file.txt"), "after\n");
        },
        dispose() {},
      };
  ...
  assert.deepEqual(
    "orchestrationTrace" in result && result.orchestrationTrace?.map((item) => item.role),
    ["mechanical-executor", "executor"],
  );
  assert.deepEqual(sessions, [
...
test("discover runs fixed stages, propagates prior evidence, and parses experiment conclusion", async () => {
        ...
        subscribe(listener) {
          const output =
            stage === 0
              ? JSON.stringify({
                  evidencePlan: {
                    unknowns: ["unknown behavior"],
                    sources: ["workspace"],
                    acceptanceCriteria: ["evidence found"],
                    budget: 1,
                  },
                  evidencePack: {
                    claims: [
                      {
                        claim: "repository evidence exists",
                        evidenceIds: ["repo-1"],
                        confidence: "high",
                      },
                    ],
              ...
              : stage === 1
                ...
                : JSON.stringify({
                    featureDefinition: {
                      ...
                      nonGoals: ["automatic productization"],
                    },
                    decisionLedger: [
                      {
                        decision: "ship minimal",
  ...
  assert.equal("success" in result && result.success, true);
  assert.deepEqual(
    "discovery" in result ? result.discovery?.stages.map((stage) => stage.stage) : [],
  ...
  assert.equal(
    "discovery" in result &&
      result.discovery?.stages[1]?.verification.includes("clean-replay-passed"),
    true,
  );
...
test("plan accepts only a verified and final-gated DiscoveryResult handoff", async () => {
  ...
  await finishJob(workspace, discovery.id, {
    ...
    status: "succeeded",
    success: true,
    output: "approved discovery",
    model: "test-provider/test-model",
    changedFiles: [],
    diffStat: "",
    verification: { status: "passed", commands: ["all-stages"] },
    discovery: {
      stages: [
        {
          stage: "research",
        ...
        },
        {
          ...
          output: "experiment",
          evidence: [],
          verification: ["clean-replay-passed"],
        },
        {
          stage: "convergence",
          status: "passed",
          output: "definition",
          evidence: [],
          verification: ["schema", "user-gate:approved"],
        },
      ],
...
test("background mechanical implementation uses an isolated job worktree", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async () => {
      throw new Error("background must not create a foreground session");
    },
  };
  const result = await runCommand(
    {
  ...
  await runCommand(
    { command: "jobs", jobsAction: "cancel", jobId, reconfigure: false, reset: false, json: true },
    workspace,
    dependencies,
  );
  await runCommand(
...
test(
  "supervised lenient jobs inject one shared sandbox runner",
  {
...
test("background worker rejects a tampered provider configuration snapshot", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    readFile: async () => "unused",
    createSession: async () => {
      throw new Error("tampered snapshot must not start a session");
    },
  };
...
test("jobs commands expose list, status, wait timeout, and acknowledge", async () => {
  ...
  assert.equal(
    "jobs" in listed && Array.isArray(listed.jobs) && "workerToken" in listed.jobs[0]!,
    false,
  );
...
function throwingDeps(marker: { sessions: number; reads: number }): RunnerDependencies {
  return {
    ...
    readFile: async () => {
      marker.reads += 1;
      throw new Error("input must not be read before admission");
    },
    createSession: async () => {
      marker.sessions += 1;
      throw new Error("session must not start for a rejected task");
    },
  };
}
...
test("admission rejects every task kind that the project policy disallows", async () => {
  for (const command of [
    ...
    const result = await runCommand(
      ...
      {
        spawnWorker: async () => {
          throw new Error("must not spawn a worker for a rejected task");
        },
      },
    );
...
test("admission maps profile task categories to the allowed task kinds", async () => {
  ...
  await updateState(analysis, (state) => {
  ...
  await updateState(impl, (state) => {
    state.config.profile = { tasks: ["implementation"] };
  });
  const askRej = await runCommand(
    ...
    impl,
    throwingDeps({ sessions: 0, reads: 0 }),
  );
...
test("admission never blocks non-task commands even when tasks is empty", async () => {
  ...
  for (const command of ["roles", "models", "providers", "status"] as const) {
    const result = await runCommand(
      { command, host: "claude", reconfigure: false, reset: false, json: true },
      workspace,
      deps,
    );
...
test("background durable replay ignores a later profile change", async () => {
  ...
  const submit = await runCommand(
    ...
    {
      catalog: { available: () => [fakeModel] },
      readFile: async () => "Durable question",
      createSession: async () => {
        throw new Error("no session on submit");
      },
    },
  ...
  const worker = await runCommand(
    {
      command: "__worker",
      jobId,
      workerToken: (await getJob(workspace, jobId)).job.workerToken!,
      reconfigure: false,
      reset: false,
      json: true,
    },
    workspace,
    {
      ...
      createSession: async () => ({
        subscribe(listener) {
          listener({
            type: "message_update",
            assistantMessageEvent: { type: "text_delta", delta: "replayed" },
          });
          listener({ type: "message_end", message: { role: "assistant", stopReason: "stop" } });
          return () => {};
        },
...
async function scopedImplementFixture(mutate: (workspace: string) => void) {
  ...
  await updateState(workspace, (state) => {
    state.config.profile = { dirs: ["src"], tasks: ["implementation"] };
  });
  const dependencies: RunnerDependencies = {
    catalog: { available: () => [fakeModel] },
    ...
    createSession: async (options) => ({
      subscribe(listener) {
        if (options.mode === "readonly") {
          listener({
            type: "message_update",
            assistantMessageEvent: { type: "text_delta", delta: "VERIFIED: ok" },
          });
        }
        listener({ type: "message_end", message: { role: "assistant", stopReason: "stop" } });
        return () => {};
...
test("the durable background path re-checks task-kind admission on a valid snapshot", async () => {
  ...
  const planOnly = await compileEffectiveProjectPolicy({
    cwd: workspace,
  ...
  const mismatched = createPolicySnapshot({
    ...
    effectiveProjectPolicy: planOnly,
  ...
  const worker = await runCommand(
    ...
    workspace,
    {
      ...
      createSession: async () => {
        sessions += 1;
        throw new Error("admission must reject before a session");
      },
    },
  );
  assert.equal("status" in worker && worker.status, "failed");
...
test("a resumed job keeps its absent project goal despite a later profile edit", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async (options) => ({
      ...
      async prompt() {
        if (options.mode === "implement")
          fs.writeFileSync(path.join(workspace, "note.txt"), "done\n");
      },
      dispose() {},
    }),
  };
  ...
  await updateState(workspace, (state) => {
    state.config.profile = { goal: "added after submission" };
  });
...
test("supervised wait uses a managed relay and returns approval without blocking the Host", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async () => {
      throw new Error("managed relay must not run the worker in the Host process");
    },
  };
  const result = await runCommand(
    {
...
test("managed relay timeout returns progress context and leaves the Job durable", async () => {
  ...
  const dependencies: RunnerDependencies = {
    ...
    createSession: async () => {
      throw new Error("worker should be detached");
    },
  };
  const result = await runCommand(
    {
