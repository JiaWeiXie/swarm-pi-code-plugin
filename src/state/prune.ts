...
type PrunePhase = "claimed" | "workspace-cleaned" | "quarantined" | "artifacts-removed";
...
interface WorkspaceAssessment {
  safe: boolean;
  reason: string;
  worktreeAction: "none" | "remove";
  branchAction: "none" | "remove" | "contained";
  workspace?: ExecutionWorkspace;
  scaffold: boolean;
}

export interface PruneJobReport {
  jobId: string;
  status: string;
  ...
  artifactBytes: number;
  actions: {
    artifacts: "retain" | "delete" | "deleted";
    worktree: "none" | "remove" | "removed" | "retain";
    branch: "none" | "remove" | "removed" | "contained" | "retain";
  };
  operationId?: string;
  error?: string;
}
...
export interface PruneOptions {
  ...
  apply: boolean;
  now?: Date;
  afterPhase?: (jobId: string, phase: PrunePhase) => void | Promise<void>;
}

export async function pruneJobs(cwd: string, options: PruneOptions): Promise<PruneReport> {
  const started = options.now ?? new Date();
  const cutoffMs = started.getTime() - options.olderThanMs;
  const stateDir = await resolveStateDir(cwd);
  if (!options.apply) return createPreview(cwd, stateDir, cutoffMs, options, started);
...
async function assessJob(
  ...
  if (!validJobId) reasons.push("unsafe-job-id");
  if (typeof job.prunedAt === "string") reasons.push("already-pruned");
  if (!PRUNABLE_STATUSES.has(String(job.status))) reasons.push("status-not-prunable");
  if (!Number.isFinite(finishedMs)) reasons.push("missing-finished-at");
  else if (finishedMs >= cutoffMs) reasons.push("newer-than-cutoff");
  if (hasPendingNotification(job)) reasons.push("pending-notification");
  if (hasPendingApproval(job)) reasons.push("pending-approval");
  if (hasPendingHostRequest(job)) reasons.push("pending-host-assistance");
  if (validJobId && (await hasLiveWorker(stateDir, job))) reasons.push("active-worker");

  let workspace: WorkspaceAssessment = {
    safe: true,
    reason: "no-workspace",
    worktreeAction: "none",
    branchAction: "none",
  ...
  if (reasons.length === 0 || operation) {
    ...
    if (!workspace.safe) reasons.push(workspace.reason);
  }
  ...
  if (jobStat && quarantineStat) reasons.push("ambiguous-prune-artifacts");
...
async function applyJobPrune(
  cwd: string,
  stateDir: string,
  report: PruneJobReport,
  options: PruneOptions,
): Promise<Pick<PruneJobReport, "artifactBytes" | "actions" | "operationId" | "reasons">> {
  ...
  if (current.phase === "claimed") {
    ...
    if (!assessment.safe) throw new Error(`Workspace became unsafe: ${assessment.reason}`);
    ...
    current = await setOperationPhase(cwd, report.jobId, "workspace-cleaned", {
      ...
      branchDisposition:
        assessment.branchAction === "remove"
          ? "removed"
  ...
  if (current.phase === "workspace-cleaned" || current.phase === "claimed") {
    ...
    if (sourceStat?.isSymbolicLink()) throw new Error("Job artifact directory is a symbolic link");
    if (sourceStat && quarantineStat) throw new Error("Both job and quarantine directories exist");
    if (sourceStat) await fs.rename(source, quarantine);
  ...
  await updateState(cwd, (state) => {
    ...
    if (index < 0) throw new Error(`Unknown job: ${report.jobId}`);
  ...
  return {
    ...
    actions: {
      ...
      branch:
        current.branchDisposition === "removed"
          ? "removed"
...
async function claimOperation(cwd: string, report: PruneJobReport): Promise<PruneOperation> {
...
async function recordOperationError(cwd: string, jobId: string, message: string): Promise<void> {
  await updateState(cwd, (state) => {
...
function createTombstone(job: JobRecord, operation: PruneOperation, prunedAt: string): JobRecord {
  return {
    id: job.id,
    status: job.status,
...
async function assessWorkspace(
  ...
  const duplicate = state.jobs.some((candidate) => {
    ...
    return (
      other?.worktree === workspace.worktree ||
      (job.kind !== "scaffold" && other?.branch === workspace.branch)
    );
  });
  if (duplicate) return unsafeWorkspace("workspace-referenced-by-another-job", workspace, job);
  if (job.kind === "scaffold") return assessScaffoldWorkspace(job, workspace);
  ...
  const branchRegistrations = registrations.filter(
    (candidate) => candidate.branch === `refs/heads/${workspace.branch}`,
  );
  ...
  if (worktreeStat) {
    ...
    if (status.trim()) return unsafeWorkspace("worktree-has-uncommitted-changes", workspace, job);
  }

  const tip = await git(root, ["rev-parse", "--verify", `refs/heads/${workspace.branch}`]).catch(
    () => undefined,
  );
  if (tip) {
    const integrated = await branchIsDisposable(root, tip.trim(), workspace.base, job);
    if (!integrated) return unsafeWorkspace("recoverable-artifact", workspace, job);
...
async function assessScaffoldWorkspace(
  job: JobRecord,
  workspace: ExecutionWorkspace,
): Promise<WorkspaceAssessment> {
  if (
...
async function branchIsDisposable(
  root: string,
  tip: string,
  base: string,
  job: JobRecord,
): Promise<boolean> {
  if (typeof job.materializedAt === "string" || tip === base) return true;
  return git(root, ["merge-base", "--is-ancestor", tip, "HEAD"])
    .then(() => true)
    .catch(() => false);
}

async function removeWorkspace(cwd: string, assessment: WorkspaceAssessment): Promise<void> {
...
function executionWorkspace(job: JobRecord): ExecutionWorkspace | undefined {
...
async function hasRecoverableArtifact(cwd: string, job: JobRecord): Promise<boolean> {
  if (typeof job.materializedAt === "string") return false;
  const result = await readJobResult(cwd, job.id);
  if (!result?.artifact?.deliverable) return false;
  const workspace = executionWorkspace(job);
  const commit = result.artifact.commit;
  if (job.kind === "scaffold" || !workspace || typeof commit !== "string" || !commit) return true;
  const root = await resolveWorkspaceRoot(cwd);
...
function hasPendingNotification(job: JobRecord): boolean {
  return (
...
function hasPendingApproval(job: JobRecord): boolean {
  return (
    typeof job.pendingApprovalId === "string" ||
    job.approvals?.some(
      (approval) => approval.status === "pending" || approval.notification === "pending",
    ) === true
  );
}

function hasPendingHostRequest(job: JobRecord): boolean {
  return (
    (job.pendingHostRequestIds?.length ?? 0) > 0 ||
    job.hostAssistanceRequests?.some((request) => request.status === "pending") === true
  );
}

async function hasLiveWorker(stateDir: string, job: JobRecord): Promise<boolean> {
  const heartbeat = await readJson<{ updatedAt?: string; pid?: number }>(
    path.join(stateDir, "jobs", job.id, "heartbeat.json"),
  );
  const heartbeatAt = Date.parse(heartbeat?.updatedAt ?? "");
  if (Number.isFinite(heartbeatAt) && Date.now() - heartbeatAt <= JOB_STALE_AFTER_MS) return true;
...
async function logicalBytes(target: string): Promise<number> {
...
async function findOrphanDirectories(stateDir: string, state: SwarmState): Promise<string[]> {
...
function pruneOperation(job: JobRecord): PruneOperation | undefined {
  const value = job.pruneOperation;
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const candidate = value as Partial<PruneOperation>;
  if (
    typeof candidate.id !== "string" ||
    !safeToken(candidate.id) ||
    !["claimed", "workspace-cleaned", "quarantined", "artifacts-removed"].includes(
  ...
  return {
    id: candidate.id,
    phase: candidate.phase as PrunePhase,
...
function requireJob(state: SwarmState, jobId: string): JobRecord {
  const job = state.jobs.find((candidate) => candidate.id === jobId);
...
function quarantinePath(stateDir: string, jobId: string, operationId: string): string {
  return path.join(stateDir, "jobs", `.prune-${jobId}-${operationId}`);
}

async function acquirePruneLock(stateDir: string): Promise<Awaited<ReturnType<typeof fs.open>>> {
...
async function staleLock(lockFile: string): Promise<boolean> {
  const [stat, value] = await Promise.all([
    fs.stat(lockFile).catch(() => undefined),
    readJson<{ pid?: number }>(lockFile),
  ]);
  if (!stat || Date.now() - stat.mtimeMs <= JOB_STALE_AFTER_MS) return false;
  return !value?.pid || !processAlive(value.pid);
}

async function gitWorktrees(cwd: string): Promise<Array<{ worktree: string; branch?: string }>> {
  ...
  return output
    ...
    .map((block) => {
      ...
      if (!worktree) throw new Error("Git returned an invalid worktree registration");
