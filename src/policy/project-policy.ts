...
export async function loadRepositoryDenyRules(cwd: string): Promise<PolicyRule[]> {
  ...
  try {
    ...
    throw new Error(
      `Invalid repository policy: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
...
export async function bindProjectPolicy(
  ...
  for (const operation of OPERATIONS) {
    ...
    roots[operation] = await Promise.all(
      policy.roots[operation].map((prefix) => canonicalPath(path.resolve(root, prefix))),
    );
  }
...
export function assertTaskAdmitted(policy: EffectiveProjectPolicy, kind: TaskKind): void {
  if (!policy.allowedTaskKinds.includes(kind)) {
    throw rejection(
      ...
      "admission",
      `Task kind '${kind}' is not allowed by this project policy`,
      policy,
    );
  }
}
...
export async function assertPathAllowed(
  ...
  try {
    ...
    throw rejection(
      "project-scope-violation",
      "preflight",
      `Path could not be resolved within an allowed ${operation} root: ${candidate}`,
      policy.effective,
      [candidate],
    );
  }
  if (
    !isInside(policy.executionRoot, canonical) ||
    !policy.roots[operation].some((root) => isInside(root, canonical))
  ) {
    throw rejection(
      "project-scope-violation",
      "preflight",
      `Path escapes an allowed ${operation} root: ${candidate}`,
      policy.effective,
      [candidate],
    );
  }
...
export async function assertChangedPathsAllowed(
  policy: BoundProjectPolicy,
  changedPaths: string[],
): Promise<void> {
  ...
  if (violations.length) {
    throw rejection(
      "project-scope-violation",
      "postflight",
      "Changed paths exceed allowed write roots",
      policy.effective,
      violations,
    );
  }
...
export function assertEffectiveProjectPolicyValid(policy: EffectiveProjectPolicy): void {
  validatePolicy(policy);
}

export function renderProjectPolicy(policy: EffectiveProjectPolicy): string {
  const roots = OPERATIONS.map(
    (operation) => `${operation}: ${policy.roots[operation].join(", ")}`,
  ).join("; ");
...
function normalizeDenyRule(value: unknown, index: number): PolicyRule {
  ...
  return {
    ...
    ...(stringArray(rule.taskKinds)
      ? { taskKinds: stringArray(rule.taskKinds) as TaskKind[] }
      : {}),
...
function canonicalRules(rules: PolicyRule[]): PolicyRule[] {
  return rules
    .map((rule) => canonicalValue(rule) as PolicyRule)
...
function validatePolicy(policy: EffectiveProjectPolicy): void {
  ...
  if (
    ...
    throw rejection(
      ...
      "materialization",
      "Project policy snapshot is invalid",
      policy,
    );
  }
}
