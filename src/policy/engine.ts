...
export class PolicyEngine {
  ...
  async authorize(action: PolicyAction, signal?: AbortSignal): Promise<PolicyDecision> {
    ...
    const missing = capabilities.filter(
      (capability) => !this.snapshot.rolePolicy.capabilities.includes(capability),
    );
    if (missing.length > 0) {
      return this.record(
        action,
        decision(
          ...
          "critical",
          capabilities,
          `Role ${this.snapshot.rolePolicy.role} does not have ${missing.join(", ")}.`,
          this.snapshot,
        ),
        fingerprint,
      );
    ...
    if (rule?.effect === "deny") {
      return this.record(
        action,
        decision("deny", "high", capabilities, `Denied by policy rule ${rule.id}.`, this.snapshot),
        fingerprint,
      );
    }
    ...
    if (lease && (await this.leases!.consume(lease))) {
      return this.record(
        action,
        decision("allow", "high", capabilities, `Allowed by lease ${lease.id}.`, this.snapshot),
        fingerprint,
      );
    }
    ...
    if (rule?.effect === "allow") {
      return this.record(
        action,
        decision("allow", "low", capabilities, `Allowed by policy rule ${rule.id}.`, this.snapshot),
        fingerprint,
      );
    }
    if (rule?.effect === "ask") {
      return this.record(
        action,
        approvalDecision(capabilities, `Policy rule ${rule.id} requires approval.`, this.snapshot),
        fingerprint,
      );
    }
    ...
    if (this.snapshot.sandboxMode === "strict") {
      const allowed =
        isReadonlyTool(action.toolName) ||
      ...
      return this.record(
        ...
        decision(
          ...
          allowed ? "low" : "high",
          capabilities,
          allowed ? "Strict read-only fast path." : "Strict mode does not expose this capability.",
          this.snapshot,
        ),
        fingerprint,
      );
    ...
    if (
      this.snapshot.sandboxMode === "lenient" ||
        ...
        decision(
          ...
          this.snapshot.sandboxMode === "full-access"
            ...
            : this.snapshot.sandboxMode === "autopilot"
              ...
              : "Allowed by lenient sandbox policy.",
          this.snapshot,
        ),
        fingerprint,
      );
    ...
    if (isReadonlyTool(action.toolName)) {
      return this.record(
        action,
        decision("allow", "low", capabilities, "Adaptive read-only fast path.", this.snapshot),
        fingerprint,
      );
    }
    ...
    if (effectAssessment.effect === "read-only") {
      return this.record(
        ...
        decision(
          ...
          "low",
          capabilities,
          "Adaptive deterministic read-only fast path.",
          this.snapshot,
        ),
        fingerprint,
      );
    ...
    if (
      ...
      return this.record(
        ...
        decision(
          ...
          "medium",
          capabilities,
          "Allowed trusted network destination.",
          this.snapshot,
        ),
        fingerprint,
      );
    ...
    if (!this.classifier) {
      const fallback =
        this.snapshot.approvalMode === "wait"
          ? approvalDecision(
              capabilities,
              "Classifier unavailable; supervisor approval is required.",
              this.snapshot,
            )
          : decision(
              "deny",
              "high",
              capabilities,
              "Classifier unavailable and no approval channel exists.",
              this.snapshot,
            );
    ...
    try {
      ...
      return this.record(
        action,
        classifierFallback(capabilities, this.snapshot, error),
        fingerprint,
...
export function assessPolicyActionEffect(action: PolicyAction): ActionEffectAssessment {
  const capabilities = capabilitiesFor(action);
  if (isReadonlyTool(action.toolName)) {
    return {
      version: 1,
...
function hardDeny(action: PolicyAction, snapshot: PolicySnapshot): PolicyDecision | null {
  if (action.toolName === "bash" && typeof action.input.command === "string") {
    ...
    if (analysis.commands.some(isPrivilegeEscalationInvocation)) {
      return decision(
        ...
        "critical",
        ["shell.execute"],
        "Privilege escalation commands are immutable denials.",
        snapshot,
      );
    }
    ...
    if (hasGitWrite && !autopilotAllowsOutward(snapshot, "git")) {
      return decision(
        ...
        "critical",
        ["shell.execute"],
        "Git delivery commands are immutable denials.",
        snapshot,
      );
    }
    if (hasDeployment && !autopilotAllowsOutward(snapshot, "deploy")) {
      return decision(
        ...
        "critical",
        ["shell.execute"],
        "Deployment commands are immutable denials.",
        snapshot,
      );
    }
    if (hasGitWrite || hasDeployment) {
      ...
      return approvalDecision(
        ...
        hasGitWrite
          ...
          : "Autopilot deployment requires human approval.",
        snapshot,
      );
    }
    if (analysis.commands.some(isHostProvisioningInvocation)) {
      return decision(
        ...
        "critical",
        ["shell.execute"],
        "Global package and host provisioning commands are immutable denials.",
        snapshot,
      );
    }
    if (
      snapshot.rolePolicy.role === "scaffolder" &&
      ...
      return decision(
        ...
        "high",
        ["shell.execute"],
        "Dependency installation belongs to the supervised environment-engineer phase.",
        snapshot,
      );
    }
    if (
      snapshot.rolePolicy.role === "environment-engineer" &&
      analysis.commands.some(
        (invocation) =>
          isDependencyInstallInvocation(invocation) &&
      ...
      return approvalDecision(
        ["shell.execute"],
        "Package lifecycle and native build execution requires supervisor approval.",
        snapshot,
      );
    }
    if (shellPathOperands(analysis).some(isProtectedShellPath)) {
      return decision(
        ...
        "critical",
        ["shell.execute"],
        "Shell access to protected control and credential paths is denied.",
        snapshot,
      );
    }
    ...
    if (
      ...
      return approvalDecision(
        ["shell.execute"],
        "Workspace-external shell paths require supervisor approval.",
        snapshot,
      );
    }
  ...
  if (action.domain && isForbiddenDomain(action.domain)) {
    return decision(
      ...
      "critical",
      ["network.connect"],
      "Local, private, and metadata destinations are immutable denials.",
      snapshot,
    );
  }
  if (action.path) {
    ...
    if (relative === ".." || relative.startsWith(`..${path.sep}`) || path.isAbsolute(relative)) {
      return decision(
        "deny",
        "critical",
        capabilitiesFor(action),
        "Path is outside the assigned workspace.",
        snapshot,
      );
    }
    if (
      ...
      return decision(
        ...
        "critical",
        capabilitiesFor(action),
        "Git metadata and plugin control state are immutable denials.",
        snapshot,
      );
    }
  }
...
function matchingRule(action: PolicyAction, capabilities: Capability[], snapshot: PolicySnapshot) {
  const matches = snapshot.adaptivePolicy.rules.filter(
    (rule) =>
      ...
      (!rule.pathPrefix ||
        Boolean(
          action.path &&
          isWithin(
            path.resolve(action.cwd, rule.pathPrefix),
            path.resolve(action.cwd, action.path),
          ),
        )) &&
      (!rule.domain || Boolean(action.domain && trustedDomain(action.domain, [rule.domain]))),
  );
  return (
    matches.find((rule) => rule.effect === "deny") ??
...
function validateClassifierDecision(
  ...
  const normalized =
    claimedCapabilities.length !== capabilities.length ||
    claimedCapabilities.some((item) => !capabilities.includes(item));
  const materialUnexpectedCapabilities = claimedCapabilities.filter(
    (item) =>
      !capabilities.includes(item) && item !== "filesystem.read-workspace" && item !== "git.read",
  );
  ...
  if (value.risk === "critical" && value.decision !== "deny") {
    return {
      ...decision(
        ...
        "critical",
        capabilities,
        "Critical classifier decisions cannot be approved.",
        snapshot,
      ),
      classifierEvidence,
    };
  ...
  if (value.risk === "high" && value.decision === "allow") {
    return {
      ...approvalDecision(
        capabilities,
        value.reason || "High-risk action requires approval.",
        snapshot,
        value.model,
      ),
      classifierEvidence,
  ...
  if (materialUnexpectedCapabilities.length > 0 && value.decision === "allow") {
    return {
      ...approvalDecision(
        capabilities,
        `Classifier reported an unproven material effect: ${materialUnexpectedCapabilities.join(", ")}.`,
        snapshot,
        value.model,
      ),
      classifierEvidence,
...
function isPrivilegeEscalationInvocation(invocation: ShellCommandInvocation): boolean {
  return invocation.executable === "sudo" || invocation.executable === "su";
}

function isGitWriteInvocation(invocation: ShellCommandInvocation): boolean {
  if (invocation.executable !== "git") return false;
  const parsed = parseGitInvocation(invocation.args);
  if (!parsed) return false;
  if (
    ["add", "commit", "checkout", "switch", "merge", "rebase", "reset", "push"].includes(
      parsed.subcommand,
    )
  ) {
    return true;
  }
  if (parsed.subcommand === "branch") return !isReadOnlyGitBranch(parsed.args);
  ...
  if (parsed.subcommand === "worktree") return parsed.args[0] !== "list";
  return false;
}

function isDeploymentInvocation(invocation: ShellCommandInvocation): boolean {
  ...
  return (
    invocation.executable === "terraform" && ["apply", "destroy"].includes(invocation.args[0] ?? "")
  );
}
 ...
 */
function autopilotAllowsOutward(snapshot: PolicySnapshot, kind: "git" | "deploy"): boolean {
 ...
 */
function autopilotAllowsRoutineShell(snapshot: PolicySnapshot): boolean {
  return snapshot.sandboxMode === "autopilot" || snapshot.sandboxMode === "full-access";
}

function isHostProvisioningInvocation(invocation: ShellCommandInvocation): boolean {
  if (["brew", "apt", "apt-get", "dnf", "yum", "pacman"].includes(invocation.executable)) {
    return invocation.args.some((arg) => arg === "install" || arg === "upgrade");
  }
  if (!["npm", "pnpm"].includes(invocation.executable)) return false;
...
function isDependencyInstallInvocation(invocation: ShellCommandInvocation): boolean {
...
function shellPathEscapesWorkspace(value: string, cwd: string): boolean {
...
function shellHumanReviewReason(analysis: ShellCommandAnalysis): string | null {
  if (
    analysis.malformed ||
  ...
  if (
    analysis.commands.some((invocation) =>
      [
        "rm",
        "rmdir",
        "unlink",
  ...
  if (
    analysis.commands.some(
      (invocation) =>
        ["git", "find", "rg", "fd", "sed", "sort", "tree", "file", "diff", "cmp", "tail"].includes(
          invocation.executable,
        ) && !isReadOnlyShellInvocation(invocation),
    )
  ) {
  ...
  if (
    analysis.commands.some((invocation) => {
      if (invocation.executable !== "command") return false;
...
function isBuildOrTestInvocation(invocation: ShellCommandInvocation): boolean {
  if (
    ["make", "ninja", "gradle", "gradlew", "mvn", "bazel", "xcodebuild"].includes(
      invocation.executable,
    )
  ) {
    return true;
  ...
  if (["npm", "pnpm", "yarn", "bun"].includes(invocation.executable)) {
    return invocation.args.some((arg) =>
      ["test", "run", "exec", "build", "install", "i", "ci", "add"].includes(arg),
    );
  }
  if (invocation.executable === "cargo") {
    return invocation.args.some((arg) =>
      ["test", "build", "check", "run", "install"].includes(arg),
    );
  }
  if (["pytest", "go", "dotnet", "swift", "swiftc"].includes(invocation.executable)) return true;
  return false;
}
...
function approvalDecision(
  ...
  return decision(
    snapshot.approvalMode === "wait" ? "require-approval" : "deny",
    "high",
    capabilities,
    reason,
    snapshot,
