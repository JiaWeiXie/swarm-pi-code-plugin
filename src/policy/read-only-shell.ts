...
export function shellPathOperands(analysis: ShellCommandAnalysis): string[] {
  return [
...
export function analyzeShellCommand(command: string): ShellCommandAnalysis {
  ...
  const commands: ShellCommandInvocation[] = [];
  const operators: string[] = [];
  const redirectionTargets: string[] = [];
  let current: ShellCommandInvocation | undefined;
 ...
 */
export function isReadOnlyShellCommand(command: string, cwd?: string): boolean {
  const value = command.trim();
  if (!value || value.length > MAX_READ_ONLY_COMMAND_LENGTH) return false;
...
export function isReadOnlyShellInvocation(invocation: ShellCommandInvocation): boolean {
  const { executable, args } = invocation;
  if (executable === "pwd") return args.length === 0 || (args.length === 1 && args[0] === "--");
  if (executable === "git") return isReadOnlyGit(args);
  if (["rustc", "cargo", "node", "npm", "pnpm", "yarn", "bun", "mise"].includes(executable)) {
    return (
      args.length > 0 && args.every((arg) => ["--version", "-V", "-Vv", "--verbose"].includes(arg))
    );
  }
  if (executable === "cmp") {
  ...
  if (executable === "file") return !hasAnyOption(args, ["-C", "--compile"]);
  if (executable === "sed") return isReadOnlySed(args);
  if (executable === "tail" && hasOptionPrefix(args, ["-f", "--follow", "-F"])) return false;
  if (
    [
      "ls",
      "cat",
...
export function isVersionProbeInvocation(invocation: ShellCommandInvocation): boolean {
  return (
    ["rustc", "cargo", "node", "npm", "pnpm", "yarn", "bun", "mise"].includes(
...
function isReadOnlyGit(args: string[]): boolean {
  ...
  if (!parsed || parsed.unsafeGlobalOption) return false;
  ...
  if (subcommand === "tag") return isReadOnlyGitTag(commandArgs);
  if (subcommand === "worktree") return commandArgs[0] === "list";
  if (subcommand === "remote") {
    if (commandArgs.every((arg) => arg === "-v" || arg === "--verbose")) {
      return true;
    }
    if (commandArgs[0] === "get-url") return true;
    return commandArgs[0] === "show" && commandArgs.some((arg) => arg === "-n");
  }
  return false;
}

export function isReadOnlyGitBranch(args: string[]): boolean {
  if (args.length === 0) return true;
  if (
    hasOptionPrefix(args, [
      "-d",
      "-D",
      "-m",
      "-M",
...
export function parseGitInvocation(
  args: string[],
): { subcommand: string; args: string[]; unsafeGlobalOption: boolean } | null {
  let index = 0;
...
function isReadOnlySed(args: string[]): boolean {
  ...
  if (!/^\d+(?:,\d+)?p$/.test(args[1] ?? "")) return false;
  return args.slice(2).every((arg) => !arg.startsWith("-"));
}

function pathArguments(invocation: ShellCommandInvocation): string[] {
  const { executable, args } = invocation;
  if (["echo", "printf", "true", "false", "type", "command"].includes(executable)) {
    return [];
  }
  if (["grep", "rg"].includes(executable)) return searchPathArguments(args);
  if (executable === "jq") {
    const positional = positionalArguments(args);
    return positional.slice(1);
  }
  if (executable === "sed") return args.slice(2);
  return args;
}

function searchPathArguments(args: string[]): string[] {
  const paths: string[] = [];
...
function safeWorkspacePath(value: string, cwd?: string): boolean {
...
function hasOptionPrefix(args: string[], options: string[]): boolean {
  return args.some((arg) =>
    options.some(
      (option) =>
        arg === option ||
...
function lexShell(command: string): {
  tokens: ShellToken[];
  hasExpansion: boolean;
  hasCommandSubstitution: boolean;
  hasBackticks: boolean;
  ...
  let word: ShellWord = { value: "", quoted: false };
  ...
  const hereDocDelimiters: Array<{ value: string; stripTabs: boolean }> = [];
  let pendingHereDocStripTabs = false;

  const pushWord = () => {
    if (!word.value && !word.quoted) return;
  ...
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (quote === "double") {
      if (char === '"') {
        ...
        if (char === "$") {
          hasExpansion = true;
          if (command[index + 1] === "(") hasCommandSubstitution = true;
        }
        if (char === "`") hasBackticks = true;
        word.value += char;
      }
      word.quoted = true;
      continue;
  ...
  pushWord();
  if (quote || escaped || expectHereDocDelimiter) malformed = true;
