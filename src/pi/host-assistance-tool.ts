...
export function createHostAssistanceTool(
  ...
  return {
    ...
    async execute(_toolCallId: string, params: Record<string, unknown>, signal: AbortSignal | undefined) {
      if (active) {
...
export function parseHostAssistanceRequest(input: unknown): HostAssistanceRequest {
  if (!input || typeof input !== "object" || Array.isArray(input)) throw new Error("Host Assistance request must be an object");
...
function hostContextClass(value: unknown): HostContextClass {
  if (["workspace", "web", "docs", "paper", "connector", "skill"].includes(String(value))) return value as HostContextClass;
...
function dataClassification(value: unknown): DataClassification {
  if (["public", "project-internal", "private", "secret"].includes(String(value))) return value as DataClassification;
  return "project-internal";
}

function requiredString(value: unknown, field: string, limit: number): string {
  if (typeof value !== "string" || !value.trim()) throw new Error(`Host Assistance ${field} is required`);
...
function strings(value: unknown, limit: number): string[] {
  return Array.isArray(value)
    ? value.filter((item): item is string => typeof item === "string" && Boolean(item.trim())).slice(0, limit).map((item) => item.slice(0, 2_000))
    : [];
}
