...
import {
  ...
  signOutProvider,
  stageCustomProviderCredential,
  verifyProviderConnection,
  type BuiltInProviderConnectionRequest,
...
export async function startConfigurationServer(
  ...
  const oauthSessions = new OAuthSessionManager(
    credentialVault,
    createFileCredentialStore(env.SWARM_PI_CODE_PLUGIN_AUTH_FILE),
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (request.method === "POST" && url.pathname === "/api/save") {
        ...
        const submission = body as ConfigurationSubmission;
...
function normalizeDiscoveryRequest(value: unknown): Omit<EndpointDiscoveryRequest, "apiKey"> & {
  provider: string;
...
function normalizeBuiltInConnectionRequest(value: unknown): BuiltInProviderConnectionRequest {
...
function normalizeVerificationRequest(value: unknown): {
  model: string;
  ...
  return {
    ...
    providerProfiles: record.providerProfiles as ProviderProfile[],
    ...(drafts ? { credentialDrafts: drafts as Array<{ provider: string; draftId: string }> } : {}),
  };
}
...
function objectRequest(value: unknown, label: string): Record<string, unknown> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
...
function assertNoRawCredentials(value: unknown): void {
