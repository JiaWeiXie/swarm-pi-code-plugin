...
export async function inspectReadiness(options: {
  ...
  const activeModel =
    options.modelPriority.find((model) => options.availableModels.includes(model)) ?? null;
  ...
  if (!activeModel) {
    issues.push(
      issue(
        ...
        "models",
        "blocking",
        "No configured Pi model is currently available.",
  ...
  if (
    ...
    issues.push(
      issue(
        ...
        "models",
        "warning",
        "The active model has no available fallback; delegated work cannot recover from a provider outage.",
    ...
    if (!sandbox.available) {
      issues.push(
        issue(
          ...
          "execution-safety",
          "blocking",
          sandbox.reason ?? "Sandbox backend is unavailable.",
          [
  ...
  if (workspace.disposition === "user-dirty") {
    issues.push(
      issue(
        ...
        "workspace",
        "warning",
        "The worktree contains user changes; mutation requires an isolated strategy.",
        [
  ...
  if (workspace.disposition === "unsafe") {
    issues.push(
      issue(
        ...
        "workspace",
        "blocking",
        "The workspace contains conflicts or unsafe filesystem entries.",
  ...
  const status = issues.some((entry) => entry.severity === "blocking")
    ...
    : issues.length > 0
      ? "degraded"
      : "ready";
  const commonBlocked = issues.some(
    (entry) => entry.severity === "blocking" && entry.stage !== "workspace",
  );
  const readonlyBlocked = commonBlocked || workspace.disposition === "unsafe";
  const mutationBlocked =
    readonlyBlocked || workspace.disposition === "git-unborn" || !workspace.git;
