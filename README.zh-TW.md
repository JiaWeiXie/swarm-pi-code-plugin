...
對 Bash approval 而言，只有 `shell.execute` capability 絕不會被當成唯讀。Runner 會以完整 command 經過小型、fail-closed 的 inspection grammar，拒絕 expansion、redirection、background、替代分支、路徑越界與 write／exec flags，只有通過後才標記 `trustedReadOnly`。Host 仍必須獨立檢查精確 command 與 fingerprint；無法辨識的 shell syntax 會回問使用者，不會擴張 lease。
