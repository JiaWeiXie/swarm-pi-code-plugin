...
| `swarm-pi-review` | 對 Git working tree 或 branch 提出可採取行動的 bug、安全性、regression 與缺少測試 findings。簡化、過度設計、刪除、重用、stdlib/native replacement 或 YAGNI 請求請選 `lean` profile。請提供預期 diff scope；branch review 另提供 base ref。 | 唯讀。`standard` 保持單一 reviewer 流程；`lean` 會平行執行三個候選 reviewer、去重並最多保留六項候選，再以最多三個並行 validator 獨立驗證，每次只公開 `supported` 的簡化建議。Pi findings 是待驗證證據，不是權威 review；Host 仍要對照實際 diff 確認。 |
...
`review --review-profile standard|lean` 預設為 `standard`。`lean` 是只檢查
diff 的唯讀兩輪 panel：clarity、YAGNI 與 leverage 候選會平行執行，每個保留候選再交給全新且獨立的 validator。最多使用九個 logical session（三個候選加六個 validator），因此以較高的延遲與模型用量換取已驗證的簡化 finding。只公開 `supported` 的 `delete`、`reuse`、`stdlib`、`native`、`yagni`、`clarify` 或 `shrink` finding。若仍有驗證未完成，結果會是 inconclusive 或 partial，不會輸出 `Lean already. Ship.`。
...
內部 `__worker` 指令只供 runner 的 managed relay 使用。其 `--job` 與
`--worker-token` 必須一起通過驗證；請勿手動呼叫。
