...
![尚未偵測到可用連線的六步驟設定頁](docs/assets/setup/01-empty-connections.png)
...
![ChatGPT Plus 或 Pro 訂閱 OAuth](docs/assets/setup/02-chatgpt-subscription.png)
...
![自訂 endpoint 的協定與驗證方式](docs/assets/setup/02-endpoint-discovery.png)

建立連線草稿後，編輯設定、替換憑證、驗證 API、登出與從專案移除會維持為不同操作。秘密欄位留白代表保留既有憑證；瀏覽器不會讀回已儲存的秘密。

![Provider readiness 與憑證管理操作](docs/assets/setup/03-provider-actions.png)
...
![只調整角色與安全政策的 project-only 流程](docs/assets/setup/06-project-only.png)
...
![各角色的模型與 Thinking 設定](docs/assets/setup/04-role-routing.png)

![Adaptive 沙盒與 classifier 設定](docs/assets/setup/05-execution-safety.png)
...
![Workspace 目標、範圍與可委派工作](docs/assets/setup/03-project-setup.png)

![Provider、模型、專案與安全政策 Review](docs/assets/setup/04-review.png)

儲存成功後，系統會以交易方式更新憑證與設定、清除記憶體內草稿，並關閉暫時的本機 server。這些截圖全部使用隔離的示範 workspace，不含任何真實憑證。

![設定儲存完成](docs/assets/setup/05-saved.png)
...
每個新 durable job 都會保存不含秘密的 provider/model snapshot 與完整性雜湊。之後修改設定只會影響新工作；憑證會在實際執行時重新解析，因此登出或輪替仍會對 queued job 生效。
...
核准通知與 terminal notification 分開確認。Capability lease 會綁定 job generation、policy hash、action fingerprint 與有效期限。Approve 或 deny 會在同一個 state transaction 中解決該核准並只 acknowledge 對應的核准通知；terminal notification 必須在 Host 顯示結果後另外明確確認。
