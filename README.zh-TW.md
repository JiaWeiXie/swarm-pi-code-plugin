...
目前的 P0 slice 只定義版本化的本機資料合約、privacy allowlist validator、
無副作用的 no-op `TelemetryRecorder`，以及使用靜態計價 fixture 的固定精度成本
邏輯。它不是已啟用的 collector：不會儲存或傳送事件、不會建立隱藏的家目錄資料夾
或 sidecar，也沒有 Dashboard、lifecycle instrumentation、計價刷新或 billing
整合。本機模型只保留 usage-only；stale、unknown、unsupported 與多幣別狀態都會
明確保留。先前的 sidecar discovery 結論是 `inconclusive`，因此本程式不宣稱效能
或計費準確性。詳見[telemetry 合約參考](docs/telemetry.md)。
