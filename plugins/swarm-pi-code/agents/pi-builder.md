...
1. Confirm the request explicitly asks for code or documentation changes.
...
node "${CLAUDE_PLUGIN_ROOT}/scripts/pi-runner.mjs" implement \
  --host claude --prompt-file "$PROMPT_FILE" --json
```
...
7. Report Pi model/fallback, changed files, host verification, and remaining risks.

Never commit, push, switch branches, or claim verification was run when the result says `not-run`.
