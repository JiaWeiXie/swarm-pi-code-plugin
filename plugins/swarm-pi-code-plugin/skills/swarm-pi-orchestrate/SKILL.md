...
2. Write one self-contained brief that names the decision, constraints, shared EvidencePack, and evidence required. Perspectives must not independently repeat expensive full builds or test suites. When dynamic verification is necessary, have the Host run one resource-aware bounded verification and share its evidence with every perspective. The persisted job result is the durable handoff.
...
4. Retain the Job ID after `approval-required`, Host Assistance, or `wait-timed-out`, then poll with `jobs wait --wait-timeout-ms 15000 --job <job-id> --json` until terminal. Inspect the full WorkerAssessment and adjudication context. Because perspectives are read-only, an active Host may auto-resolve only eligible public/read-only context within the snapshot; otherwise ask the user. Do not leave one opaque shell command running.
