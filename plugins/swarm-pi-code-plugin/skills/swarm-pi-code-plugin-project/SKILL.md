...
description: Reopen the guided Swarm Pi project goal, working area, and delegated task setup without changing Provider or model configuration.
---
...
4. After a successful save, run `node "$PLUGIN_ROOT/scripts/pi-runner.mjs" init --host "$HOST" --json` and report the saved project goal, working directories, and delegated task types.

This skill is safe to run repeatedly. It reads and updates only the shared project profile in `.swarm-pi-code-plugin/state.json`; it does not change Provider, model priority, credentials, or job history.
