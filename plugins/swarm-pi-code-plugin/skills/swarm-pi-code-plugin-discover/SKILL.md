...
4. Treat the three fixed reports as gates: research/evidence plan, experiment micro-SDLC, and definition/convergence. The runner schema-validates every stage. Research must pass its Human Decision gate before the experiment. The experiment runs as a durable, isolated `experimenter` child Job and cannot pass schema validation without a hypothesis, baseline/control, locked dependencies, fixture, seed/data hash, setup/run/test/verify/cleanup commands, metrics, tolerance, explicit evidence, tests, and a reported clean replay. The 0.5.0 control plane does not independently replay all of those commands.
