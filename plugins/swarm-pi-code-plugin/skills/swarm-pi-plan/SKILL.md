...
3. Retain the Job ID after `approval-required`, Host Assistance, or `wait-timed-out`, then keep the relay alive with bounded `$RUNNER jobs wait --job <id> --wait-timeout-ms 15000 --json` calls. Inspect the full WorkerAssessment and adjudication context. An active Host may auto-resolve only eligible public/read-only context within the snapshot; otherwise ask the user. The decision acknowledges only its matching notification.
