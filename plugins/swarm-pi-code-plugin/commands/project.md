---
description: Reopen the guided Swarm Pi project goal, working area, and delegated task setup
...
4. After a successful save, run `node "${CLAUDE_PLUGIN_ROOT}/scripts/pi-runner.mjs" init --host claude --json` and report the saved project goal, working directories, and delegated task types.

This command is safe to run repeatedly. It reads and updates only the shared project profile in `.swarm-pi-code-plugin/state.json`; it does not change Provider, model priority, credentials, or job history.
