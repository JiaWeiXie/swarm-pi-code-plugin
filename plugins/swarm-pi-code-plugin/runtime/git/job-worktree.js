...
import { acquireWorktreeLease, assertWorktreeBaseline, assessWorkspace, requireCleanWorktree, validateChangedPaths, } from "./worktree.js";
import { resolveWorkspaceRoot } from "../state/state.js";
const execFileAsync = promisify(execFile);
export function jobWorktreeBranch(jobId) {
    return `swarm-pi/${jobId.replace(/[^a-zA-Z0-9._-]/g, "-").slice(-48)}`;
}
export async function prepareJobWorktree(cwd, jobId, strategy = "auto") {
    const workspace = await fs.realpath(await resolveWorkspaceRoot(cwd));
    if (strategy === "auto")
        await requireCleanWorktree(workspace);
    const assessment = await assessWorkspace(workspace);
    if (assessment.disposition === "unsafe")
        throw new Error("Unsafe worktree cannot be isolated");
...
export async function checkpointJobWorktree(worktree, jobId) {
    await execFileAsync("git", ["add", "-A"], { cwd: worktree, encoding: "utf8" });
    const status = await git(worktree, ["status", "--porcelain=v1"]);
    if (!status.trim())
        return null;
...
export async function materializeJobWorktree(cwd, jobId, artifact) {
    const workspace = await fs.realpath(await resolveWorkspaceRoot(cwd));
    const assessment = await assessWorkspace(workspace);
    if (assessment.disposition !== "clean" && assessment.disposition !== "safe-dirty") {
        throw new Error(`Implementation artifact requires a clean or safe-dirty target; workspace is ${assessment.disposition}`);
    }
    ...
    const collision = changedFiles.find((file) => preserved.some((entry) => file === entry || file.startsWith(`${entry}/`)));
    if (collision)
    ...
    try {
        ...
        if (applied) {
            try {
                ...
                throw new Error(`Implementation materialization failed (${original}) and rollback also failed (${rollback})`);
            }
        }
        throw error;
    }
...
export async function cleanupJobWorktree(cwd, artifact, discard = false) {
    const workspace = await fs.realpath(await resolveWorkspaceRoot(cwd));
    if (!discard && !artifact.commit) {
        const status = await git(artifact.worktree, ["status", "--porcelain=v1"]).catch(() => "");
        if (status.trim())
            throw new Error("Job worktree contains uncommitted changes; use --discard to remove it explicitly");
    }
    if (!discard && artifact.commit) {
        try {
