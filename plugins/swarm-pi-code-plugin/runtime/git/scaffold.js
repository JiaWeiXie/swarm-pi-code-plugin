...
export async function prepareScaffoldWorkspace(cwd, target, jobId, spec) {
...
export async function checkpointScaffold(workspace, jobId) {
    await git(workspace.worktree, ["add", "-A"]);
    const status = await git(workspace.worktree, ["status", "--porcelain=v1"]);
    if (status.trim())
        await artifactCommit(workspace.worktree, `swarm-pi: scaffold ${jobId}`, false);
    ...
    try {
        ...
        if (options.stateDir) {
            ...
            if (options.moveState)
                ...
                await seedProjectState(options.stateDir, destinationState);
        }
...
async function copyAdoptionBaseline(source, destination, preserved) {
    ...
    async function visit(fromDirectory, toDirectory, prefix = "") {
        for (const entry of await fs.readdir(fromDirectory, { withFileTypes: true })) {
            const relative = prefix ? `${prefix}/${entry.name}` : entry.name;
            if (relative === ".git" || preservedSet.has(relative))
                continue;
...
async function copyPreserved(source, destination, paths) {
...
async function assertTargetIsNotLink(target) {
    ...
    if (stat?.isSymbolicLink())
        throw new Error("Scaffold target cannot be a symbolic link");
}
async function acquireMaterializationLock(parent, target) {
    ...
    while (true) {
        try {
            ...
            if (Date.now() >= deadline)
                throw new Error("Timed out waiting for materialization lock");
...
async function nearestExistingDirectory(candidate) {
    ...
    while (true) {
        if ((await fs.stat(current).catch(() => undefined))?.isDirectory())
            return current;
