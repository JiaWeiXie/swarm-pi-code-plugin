...
export async function exportJobAudit(cwd, jobId) {
    ...
    if (!snapshot.result)
        throw new Error(`Job has no result artifact: ${jobId}`);
    ...
    const counter = { secrets: 0, paths: 0 };
...
async function readSources(directory) {
    let total = 0;
    const readRequired = async (name) => {
        const file = path.join(directory, name);
        const value = await readLimited(file, false);
        if (value === null)
            throw new Error(`Missing audit source: ${name}`);
...
async function readLimited(file, optional) {
    try {
        const stat = await fs.stat(file);
        if (stat.size > MAX_AUDIT_SOURCE_BYTES)
            throw new Error("Audit source exceeds the size limit.");
...
async function redactionRoots(cwd, state) {
    return {
...
function summarizeRequest(request, roots, counter) {
    const value = {
...
function summarizeApproval(approval, roots, counter) {
    const value = {
        id: approval.id,
...
function summarizeResult(result, roots, counter) {
    const value = {
        kind: result.kind,
...
function verifyPolicySnapshot(snapshot) {
    if (!snapshot)
        return null;
...
function redactValue(value, roots, counter, key) {
    if (key && SENSITIVE_KEY.test(key)) {
        counter.secrets += 1;
        return "[redacted]";
    }
    if (typeof value === "string")
        return redactString(value, roots, counter);
    if (Array.isArray(value))
        return value.map((item) => redactValue(item, roots, counter));
...
function redactString(value, roots, counter) {
    let output = value;
    for (const [root, replacement] of [
        ...roots.workspace.map((value) => [value, "$WORKSPACE"]),
        ...roots.state.map((value) => [value, "$STATE"]),
        ...roots.home.map((value) => [value, "$HOME"]),
        ...roots.temp.map((value) => [value, "$TMP"]),
    ...
    output = output.replace(URL_USERINFO_PATTERN, (_match, protocol) => {
        counter.secrets += 1;
        return `${protocol}[redacted]@`;
    });
    return output;
}
function assertSafeExport(serialized, request, roots) {
